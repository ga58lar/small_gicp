...
/// @tparam HasLabels     If true, store labels.
template <bool HasNormals = false, bool HasCovs = false, bool HasLabels = false>
...
public:
  ...
  void add(const Setting& setting, const Eigen::Vector4d& transformed_pt, const PointCloud& points, size_t i, const Eigen::Isometry3d& T) {
    ...
    if constexpr (HasLabels) {
      this->labels.emplace_back(traits::label(points, i));
    }
  }
...
public:
  ...
  std::conditional_t<HasLabels, std::vector<int>, Empty> labels;        ///< Labels (Empty if HasLabels is false)
};

/// @brief FlatContainer that stores only points.
using FlatContainerPoints = FlatContainer<false, false, false>;
/// @brief FlatContainer with normals.
using FlatContainerNormal = FlatContainer<true, false, false>;
/// @brief FlatContainer with covariances.
using FlatContainerCov = FlatContainer<false, true, false>;
/// @brief FlatContainer with normals and covariances.
using FlatContainerNormalCov = FlatContainer<true, true, false>;
/// @brief FlatContainer with normals and labels.
using FlatContainerNormalLabel = FlatContainer<true, false, true>;
/// @brief FlatContainer with covariances and labels.
using FlatContainerCovLabel = FlatContainer<false, true, true>;
/// @brief FlatContainer with normals, covariances and labels.
using FlatContainerNormalCovLabel = FlatContainer<true, true, true>;
/// @brief FlatContainer with labels.
using FlatContainerLabel = FlatContainer<false, false, true>;

namespace traits {

template <bool HasNormals, bool HasCovs, bool HasLabels>
struct Traits<FlatContainer<HasNormals, HasCovs, HasLabels>> {
  static size_t size(const FlatContainer<HasNormals, HasCovs, HasLabels>& container) { return container.size(); }
  static bool has_points(const FlatContainer<HasNormals, HasCovs, HasLabels>& container) { return container.size(); }
  static bool has_normals(const FlatContainer<HasNormals, HasCovs, HasLabels>& container) { return HasNormals && container.size(); }
  static bool has_covs(const FlatContainer<HasNormals, HasCovs, HasLabels>& container) { return HasCovs && container.size(); }
  static bool has_labels(const FlatContainer<HasNormals, HasCovs, HasLabels>& container) { return HasLabels && container.size(); }

  static const Eigen::Vector4d& point(const FlatContainer<HasNormals, HasCovs, HasLabels>& container, size_t i) { return container.points[i]; }
  static const Eigen::Vector4d& normal(const FlatContainer<HasNormals, HasCovs, HasLabels>& container, size_t i) { return container.normals[i]; }
  static const Eigen::Matrix4d& cov(const FlatContainer<HasNormals, HasCovs, HasLabels>& container, size_t i) { return container.covs[i]; }
  static const int& label(const FlatContainer<HasNormals, HasCovs, HasLabels>& container, size_t i) { return container.labels[i]; }

  static size_t nearest_neighbor_search(const FlatContainer<HasNormals, HasCovs, HasLabels>& container, const Eigen::Vector4d& pt, size_t* k_index, double* k_sq_dist) {
    return container.nearest_neighbor_search(pt, k_index, k_sq_dist);
  }

  static size_t knn_search(const FlatContainer<HasNormals, HasCovs, HasLabels>& container, const Eigen::Vector4d& pt, size_t k, size_t* k_index, double* k_sq_dist) {
    return container.knn_search(pt, k, k_index, k_sq_dist);
  }

  template <typename Result>
  static void knn_search(const FlatContainer<HasNormals, HasCovs, HasLabels>& container, const Eigen::Vector4d& pt, Result& result) {
