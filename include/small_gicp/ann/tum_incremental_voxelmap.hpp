...
#include <small_gicp/ann/tum_flat_container.hpp>
...
struct TUMIncrementalVoxelMap {
public:
  using Ptr = std::shared_ptr<TUMIncrementalVoxelMap>;
  using ConstPtr = std::shared_ptr<const TUMIncrementalVoxelMap>;

  /// @brief Constructor.
  /// @param leaf_size  Voxel size
  explicit TUMIncrementalVoxelMap(double leaf_size) : inv_leaf_size(1.0 / leaf_size), removal_horizon(100), lru_clear_cycle(10), lru_counter(0) { set_search_offsets(1); }
...
template <typename VoxelContents>
struct Traits<TUMIncrementalVoxelMap<VoxelContents>> {
  static size_t size(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap) { return voxelmap.size(); }

  static Eigen::Vector4d point(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap, size_t i) {
  ...
  static Eigen::Vector4d normal(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap, size_t i) {
    const size_t voxel_id = voxelmap.voxel_id(i);
    const size_t point_id = voxelmap.point_id(i);
    const auto& voxel = voxelmap.flat_voxels[voxel_id]->second;
    return traits::normal(voxel, point_id);
  }
  static Eigen::Matrix4d cov(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap, size_t i) {
    const size_t voxel_id = voxelmap.voxel_id(i);
    const size_t point_id = voxelmap.point_id(i);
    const auto& voxel = voxelmap.flat_voxels[voxel_id]->second;
    return traits::cov(voxel, point_id);
  }
  static int label(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap, size_t i) {
    const size_t voxel_id = voxelmap.voxel_id(i);
    const size_t point_id = voxelmap.point_id(i);
    const auto& voxel = voxelmap.flat_voxels[voxel_id]->second;
    return traits::label(voxel, point_id);
  }

  static size_t nearest_neighbor_search(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap, const Eigen::Vector4d& pt, size_t* k_index, double* k_sq_dist) {
    return voxelmap.nearest_neighbor_search(pt, k_index, k_sq_dist);
  }

  static size_t knn_search(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap, const Eigen::Vector4d& pt, int k, size_t* k_index, double* k_sq_dist) {
    return voxelmap.knn_search(pt, k, k_index, k_sq_dist);
  }
};

template <typename VoxelContents>
std::vector<size_t> point_indices(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap) {
...
template <typename VoxelContents>
std::vector<Eigen::Vector4d> voxel_points(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap) {
...
template <typename VoxelContents>
std::vector<Eigen::Vector4d> voxel_normals(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap) {
...
template <typename VoxelContents>
std::vector<Eigen::Matrix4d> voxel_covs(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap) {
...
template <typename VoxelContents>
std::vector<int> voxel_labels(const TUMIncrementalVoxelMap<VoxelContents>& voxelmap) {
  std::vector<int> labels;
  labels.reserve(voxelmap.size() * 5);

  for (const auto& voxel : voxelmap.flat_voxels) {
    for (size_t i = 0; i < traits::size(voxel->second); i++) {
      labels.push_back(traits::label(voxel->second, i));
    }
  }
  return labels;
}
